...
import { buildTimeSlots, formatPrice } from "@/lib/mock-data";
...
export default function BookingFlow({
  ...
  if (submitted) {
    const cal = selectedService
      ? buildCalendarLinks(businessName, selectedService, selectedDate, selectedTime)
      : null;

    return (
      <div className="min-h-screen bg-ivory flex flex-col">
        {/* Step 5 of 5 — the flow's final, confirmed state */}
        <BookingHeader step={5} businessName={businessName} slug={slug} />
  ...
  return (
    <div className="min-h-screen bg-ivory flex flex-col">
      <BookingHeader step={stepNumber} businessName={businessName} slug={slug} />
 ...
 */
function buildCalendarLinks(businessName: string, service: Service, dateISO: string, timeHM: string) {
  ...
  const title = `${service.name} в ${businessName}`;
  const details = `Ваша запись на ${service.name} в ${businessName}.`;
